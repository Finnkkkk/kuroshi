...
├── .env.example
├── src/
│   ├── app.js               # monta as rotas Express recebendo o provedor como parâmetro (testável)
│   ├── server.js             # ponto de entrada: escolhe o provedor por env var e sobe o servidor
...
└── test/
    └── app.test.js            # testa as rotas com um provedor falso (sem rede real)
```
...
Os testes usam um provedor falso (veja `test/app.test.js`) — não fazem
nenhuma chamada real pra Jikan/AniList, então rodam rápido e de forma
determinística em qualquer ambiente, inclusive CI.
...
```js
const CONFIG = {
  DATA_SOURCE: "backend",                  // era "jikan" (o padrão, direto do navegador)
...
**Isso só é necessário se você quiser usar a AniList, ou seu próprio
backend com cache/rotas extras** — o modo padrão (`DATA_SOURCE: "jikan"`)
já busca dados reais direto da Jikan a partir do navegador, sem precisar
rodar nada disso.
